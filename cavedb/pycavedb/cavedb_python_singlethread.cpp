...
{
	PyThreadState *c;
	state_saver(PyThreadState* mainThreadState):c(mainThreadState)
	{
		PyEval_AcquireThread( c);
	}
	~state_saver()
	{
		PyEval_ReleaseThread( c);
	}
};
...
void cave_ssdb_thread(void* param)
{
	...
	tp->store->threadState = tp->mainThreadState;
...
static PyObject * slaveof_ssdb(PyObject *self, PyObject *args)
{
	...
	tp->store = new python_cavedb_store(self,tp->mainThreadState);
